...
assert.match(app, /function setDashboardView\(view\)/);
assert.match(app, /No activity yet\./);
assert.match(app, /data-label="Filename"/);
assert.doesNotMatch(app, /Loading settings panel\.\.\./);
assert.match(styles, /\.dashboard-page \.db-stats-grid[\s\S]*grid-template-columns:\s*repeat\(2/);
assert.match(styles, /\.dashboard-history-table thead\s*\{\s*display:\s*none/);
assert.match(styles, /\.contact-submit-row[\s\S]*position:\s*sticky/);
...
assert.equal((app.match(/Presentation renderer unavailable/g) || []).length, 1);
assert.match(app, /Upload and conversion controls are disabled until that renderer is available\./);
